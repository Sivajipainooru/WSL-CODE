...
//		ele.click();
	...
	public void majorDetails(String fn,String ln,String db)
	{
		...
		for(int i=0;i<passportdd.size();i++)
		{
			if(passportdd.get(i).getText().equals("India"))
			{
	...
	public void minorDetails(String fn1,String ln1,String db1,String rship) throws InterruptedException
	{
		...
		waitForElement(Dropdownpassportcoutry);
		List<WebElement> passPort = driver.findElements(Dropdownpassportcoutry);
		for(int i=0;i<passPort.size();i++)
		{
			System.out.println(passPort.get(i).getText());
			if(passPort.get(i).getText().equals("India"))
			{		
				passPort.get(i).click();
				break;				
			}
		}
...
//		a.moveToElement(paymoney).click().perform();
		//Actions action=new Actions(driver);
		//action.keyDown(Keys.click()).build().perform();
	}

	
//	static XSSFSheet sheet;
//	private static String getCellValue(int rowss, int cells)
//	{
//		XSSFRow rows=sheet.getRow(rowss);
//		XSSFCell cell=rows.getCell(cells);
...
//	{ 
//		FileInputStream f            = new	FileInputStream("D:\\WSLXLSHEETS\\AllBookAppointmentDetails.xlsx"); 
//		XSSFWorkbook wb              = new XSSFWorkbook(f); 
//		XSSFSheet sheet              = wb.getSheetAt(0); 
//		 row                      = sheet.getLastRowNum(); 
//		 cell                   = sheet.getRow(row).getLastCellNum();
